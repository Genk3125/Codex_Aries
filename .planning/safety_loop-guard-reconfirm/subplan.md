...
- [ ] loop_guard.py のパラメータが recovery-playbook.md と整合
- [ ] poc/ 内に自動再試行パターンが混入していないことを grep で確認
- [ ] now-3 dogfooding 中に guard が期待通りに動作することを実地確認
- [ ] 差分があった場合は修正完了、なければ PASS 記録

## 6. リスク・注意

- grep だけでは検出漏れの可能性あり（変数名が異なる場合）→ Step 3 の実地確認で補完
- recovery-playbook.md 自体の記述が曖昧な場合 → playbook 側を先に明確化
- 「single-pass を壊す」の定義が曖昧な場合 → 「1 回の orchestrator 実行で同一 helper を 2 回以上呼ばない」を基準とする
